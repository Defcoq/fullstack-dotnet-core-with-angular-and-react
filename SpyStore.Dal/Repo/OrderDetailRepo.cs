...
namespace SpyStore.Dal.Repo
{
    public class OrderDetailRepo : RepoBase<OrderDetail>, IOrderDetailRepo
    {
        ...
        public IEnumerable<OrderDetailWithProductInfo> GetOrderDetailsWithProductInfoForOrder(int orderId)=> Context.OrderDetailWithProductInfos.Where(x => x.OrderId == orderId).OrderBy(x => x.ModelName);
    }
}
