...
namespace SpyStore.Dal.Repo
{
    public class ShoppingCartRepo : RepoBase<ShoppingCartRecord>, IShoppingCartRepo
    {
        ...
        public IEnumerable<CartRecordWithProductInfo>
        GetShoppingCartRecords(int customerId) => Context.CartRecordWithProductInfos
        .Where(x => x.CustomerId == customerId)
        .OrderBy(x => x.ModelName);

        public CartWithCustomerInfo GetShoppingCartRecordsWithCustomer(int customerId) =>new CartWithCustomerInfo()
            {
        ...
        ShoppingCartRecord entity, Product product, bool persist = true)
        {
            ...
            "Record was changed since it was loaded");
        }
        ...
        IEnumerable<ShoppingCartRecord> entities, bool persist = true)
        {
            int counter = 0;
            foreach (var item in entities)
            {
